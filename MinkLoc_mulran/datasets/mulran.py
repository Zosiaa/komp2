...
class MulRanDataset(Dataset):
    ...
    def __init__(self, dataset_path, query_filename, transform=None, set_transform=None, max_elems=None):
        ...
        self.n_points = 65536   # pointclouds in the dataset are downsampled to 4096 points
    ...
    def preprocess_queries(self, query_filepath, cached_query_filepath):
        ...
        print("pickle in RAM") #named queries
        # Convert to bitarray
        i=0
        for ndx in tqdm.tqdm(queries):
            #print(queries[i]['positives'])
            queries[i]['positives'] = set(queries[i]['positives'])
            set2 = set(map(int, queries[i]['positives']))
            queries[i]['negatives'] = set(queries[i]['negatives'])
            set3 = set(map(int, queries[i]['negatives']))
            #pos_mask = self.get_positives_mask(queries)
            pos_mask = [queries[e_ndx]['query'] in set2 for e_ndx in range(len(queries))]
            neg_mask = [queries[e_ndx]['query'] in set3 for e_ndx in range(len(queries))]
            queries[i]['positives'] = bitarray(pos_mask)
            queries[i]['negatives'] = bitarray(neg_mask)
            i+=1
    ...
    def __getitem__(self, ndx):
        ...
        print(self.queries[ndx]['query'])
        print("------------")
        filename = str(self.queries[ndx]['query'])
    ...
    def load_pc(self, filename):
        ...
        # Returns Nx3 matrix
        file_path = self.dataset_path + "/" + filename + ".bin"
        ...
        pc = np.reshape(pc, (pc.shape[0] // -1, 4))[:, 0:3]
