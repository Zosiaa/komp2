...
from datasets.mulran import MulRanDataset, TrainTransform, TrainSetTransform
...
def make_datasets(params: MinkLocParams, debug=False):
    ...
    datasets['train'] = MulRanDataset(params.dataset_folder, params.train_file, train_transform,
                                      set_transform=train_set_transform, max_elems=max_elems)
    val_transform = None
    if params.val_file is not None:
        datasets['val'] = MulRanDataset(params.dataset_folder, params.val_file, val_transform)
    return datasets


def make_eval_dataset(params: MinkLocParams):
    # Create evaluation datasets
    dataset = MulranDataset(params.dataset_folder, params.test_file, transform=None)
    return dataset


def make_collate_fn(dataset: MulRanDataset, mink_quantization_size=None):
    ...
    def collate_fn(data_list):
        ...
        if mink_quantization_size is None:
            ...
            coords = [ME.utils.sparse_quantize(e, quantization_size=mink_quantization_size)
                      for e in batch]
...
def make_dataloaders(params: MinkLocParams, debug=False):
    ...
    :return:
    """
    datasets = make_datasets(params, debug=debug) #change to Mulran
